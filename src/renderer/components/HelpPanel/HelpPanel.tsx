  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: '角色登场与退场',
        content: [
          '用 begin(1) 让角色"登场"，用 begin(0) + end() 让角色"退场"。',
          '',
          'begin(1)：自动管理——角色用完不用操心，脚本结束了自己会消失。',
          '  适合临时出现的角色，比如路人。',
          '',
          'begin(0) + end()：需要手动让角色退场。',
          '  适合贯穿始终的主角，想什么时候让他走就什么时候让他走。',
          '',
          '示例：',
    ...
    items: [
      {
        ...
        title: '切换背景',
        content: [
          '用 Background.set 创建背景，再 begin(1) 让它显示出来。',
          '新的背景会覆盖旧的背景。',
          '',
          '示例：',
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        title: '生命周期',
        content: [
          'begin(mode=0)：启用资源，角色出现在舞台上。',
          '  mode=0 - 手动管理，需调用 end() 销毁',
          '  mode=1 - 自动销毁，脚本块执行完毕后自动释放',
          '',
          'end()：销毁资源，释放内存。',
          '',
          '使用场景建议：',
...
const API_SECTIONS: ApiSection[] = [
    ...
    subsections: [
      {
        title: '生命周期',
        rows: [
          { name: 'begin', signature: 'obj.begin(mode?: num)', desc: '启用对象（0=手动, 1=自动销毁）' },
          { name: 'hide', signature: 'obj.hide()', desc: '隐藏但不销毁' },
  ...
  },
  {
    ...
    rows: [
      ...
      { name: 'blur', signature: 'filter.blur(val: num)', desc: '高斯模糊（0~2.0）' },
      { name: 'brightness', signature: 'filter.brightness(val: num)', desc: '明度（0~2.0）' },
      { name: 'contrast', signature: 'filter.contrast(val: num)', desc: '对比度（0~2.0）' },
      { name: 'saturation', signature: 'filter.saturation(val: num)', desc: '饱和度（0~2.0）' },
      { name: 'gamma', signature: 'filter.gamma(val: num)', desc: '伽马（0~10）' },
